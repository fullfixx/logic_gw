...
if (count($_GET) > 0) {
    ...
    $query = "SELECT * FROM `gas` INNER JOIN `supplier_list` ON `gas`.`gas_supplier`=`supplier_list`.`supplier_id` WHERE `gas_sold` >= 3 AND `gas_supplier` = '$gas_supplier'";
    $backorders = mysqli_query($link, $query);
} else {
    $query = "SELECT * FROM `gas` INNER JOIN `supplier_list` ON `gas`.`gas_supplier`=`supplier_list`.`supplier_id` WHERE `gas_sold` >= 3";
    $backorders = mysqli_query($link, $query);
}
...
<main>
        ...
        <div class="container-fluid">
            ...
            <div class="row">
                <div class="col-4 text-left">
                    <a href="backorders.php" class="text-dark"><h3 class="mb-1 mt-2">ЗАКАЗЫ ЗАПЧАСТЕЙ</h3></a>
                </div>
                <div class="col-4">

                </div>
                <div class="col-4">
                    <form action="backorders.php" method="get" class="my-1">
            ...
            <table class="table table-sm table-bordered table-striped table-hover">
                ...
                <tr>
                    <th class="py-1 px-1 align-middle text-center">
                        <?php
                        $entered_user = $_SESSION['user']['rank'];
                        if ($entered_user == 2) {
                            ?>
                            <button type="button" class="btn btn-warning btn-sm" data-toggle="modal" data-target="#addBackOrder">&#10010;</button>
                            <?php
                        }
                        ?>
                    </th>
                ...
                for($i=1; $out = mysqli_fetch_assoc($backorders); $i++)
                {
                    ...
                    ?>
                    <tr>
                        <td class="py-0 align-middle" title="ID: <?php echo $gas_id; ?>"><small><?php echo $i; ?></small></td>
                        <form action="upd_backorders.php" method="post">
                            <input type="hidden" name="gas_id" value="<?php echo $gas_id; ?>">
                        <td class="py-0 align-middle"><?php echo $out['gas_num']; ?></td>
                        <td class="w-25 py-0 px-0 align-middle"><input type="text" name="gas_name" class="form-control form-control-sm text-left" value="<?php echo $out['gas_name']; ?>" onchange="submit();"></td>
                        <td class="py-0 px-0 align-middle"><input type="text" name="cost" size="9" class="form-control form-control-sm text-right" value="<?php echo $out['cost']; ?>" onchange="submit();"></td>
                        <td class="py-0 px-0 align-middle"><input type="text" name="price" size="9" class="form-control form-control-sm text-right" value="<?php echo $out['price']; ?>" onchange="submit();"></td>
                        <td class="py-0 px-0 align-middle"><input type="text" name="gas_qty" size="9" class="form-control form-control-sm text-right" value="<?php echo $out['gas_qty']; ?>" onchange="submit();"></td>
                        <?php
    ...
    INNER JOIN `device_model_list` ON `orders_list`.`device_model`=`device_model_list`.`device_model_id` WHERE `order_num` = '$order_n'";
                            ...
                            foreach ($result_orders_list as $item) {
                                ?>
                                <td class="py-0 align-middle"><?php echo $item['device_brand_name']; ?></td>
                                <td class="py-0 align-middle"><?php echo $item['device_model_name']; ?></td>
                                <?
                                }
                            ?>
                        <td class="py-0 align-middle table-<?php if ($out['gas_sold'] == 3) {
                            echo "danger";
                        ...
                        <td class="py-0 align-middle"><?php if ($out['gas_sold'] == 3) {
                            ?>
                        ...
                        <td class="py-0 align-middle"><?php echo date('d.m', strtotime($out['gas_time_add'])); ?></td>
                        ...
                        <td class="py-0 align-middle"><input type="text" name="gas_comment" class="form-control form-control-sm text-left" value="<?php echo $out['gas_comment']; ?>" onchange="submit();" title="<?php echo $out['gas_comment']; ?>"></td>
                        </form>
                    </tr>
                    <?php
                }
